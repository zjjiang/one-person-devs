"""Briefing stage: generate a concise coding brief for lightweight mode."""

from __future__ import annotations

import logging

from opd.engine.context import build_briefing_prompt
...
class BriefingStage(Stage):
    """Generate a concise coding brief from the story's raw input via AI."""
    ...
    async def validate_preconditions(self, ctx: StageContext) -> list[str]:
        ...
        if not ctx.story.raw_input:
            errors.append("Story raw_input is required for briefing generation")
        return errors
    ...
    async def execute(self, ctx: StageContext) -> StageResult:
        ...
        system_prompt, user_prompt = build_briefing_prompt(ctx.story, ctx.project)
        work_dir = str(resolve_work_dir(ctx.project))

        collected: list[str] = []
        async for msg in ai.provider.prepare_prd(system_prompt, user_prompt, work_dir):
            if ctx.publish:
        ...
        if not prd_text.strip():
            return StageResult(success=False, errors=["AI returned empty briefing"])
